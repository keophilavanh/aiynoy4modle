...
class room_type_model extends MY_Model{
    ...
    var $table = "tb_room_type";  
    // var $select_column = array("sel_id", "sel_date", "Payment_status", "sel_id");  
    var $order_column = array("room_type_id", "room_name_type","price");  
    function make_query()  
    {  
         $this->db->select('*');  
         ...
         {  
              $this->db->like("room_type_id", $_POST["search"]["value"]);  
              $this->db->or_like("room_name_type", $_POST["search"]["value"]);  
    ...
    function add_room_type($info)
     ...
     function Edit_room_type($info,$id)
     {
          $this->db->where('room_type_id', $id);
