...
class Follow_account extends CI_Controller{
    ...
    public function Follow_account($id){
        if($this->Users_model->check_token())
        {
             $data['bank_sub_account'] = $this->Follow_account_model->select_item_list($id);
             $data['bank_sub_account_id']= $id;
           
            $this->load->view('conten/accounting/Bank_of_cash/view_Follow_account',$data,false);
        }
    }
...
//    } 
    ...
    public function additem(){
        ...
        if( $_POST["status"] =='income'){

            $data = array(
                'bank_sub_account_id' => $_POST["bank_sub_account_id"],
                'description' => $_POST["description"],
                'Date' => date("Y-m-d"),
                'Lift_balances' => $_POST["Lift_balances"],
                'income' => $_POST["monney"],
               
                'Still_have' => ($_POST["monney"] + $_POST["Lift_balances"]),
                ...
                $info = array(
                    ...
                    'total' =>  ($_POST["Lift_balances"] + $_POST["monney"]),
                    
                  
                    );

                $this->bank_sub_account_model->update_total($_POST["bank_sub_account_id"],$info);

            echo json_encode($this->Follow_account_model->additem($data));

        }else if($_POST["status"] =='payment'){
            ...
            $data = array(
                ...
                'payment' => $_POST["monney"],
                'Still_have' =>  ($_POST["Lift_balances"] - $_POST["monney"]),
